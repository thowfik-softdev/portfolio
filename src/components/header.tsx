  ...
  return (
    <header className="w-full border-b border-border bg-background/95 backdrop-blur-sm sticky top-0 z-50">
      ...
      <div className="max-w-5xl mx-auto px-6 py-6 text-center">
        ...
        <p className="font-serif text-sm md:text-base text-muted-foreground mt-2 italic">
          {siteConfig.tagline}
        </p>
      </div>
      <div className="editorial-rule-double max-w-5xl mx-auto" />
      ...
      <nav className="max-w-5xl mx-auto px-6 py-3">
        <ul className="flex items-center justify-center gap-6 md:gap-10 text-xs tracking-[0.2em] uppercase font-medium">
          {navLinks.map((link) => (
            <li key={link.href}>
              <a
                href={link.href}
